...
sub prefs {
	return ($prefs, qw(apcparentfolderpath playedtreshold_percent dbpopminplaycount alwaysdisplayvals));
}
...
sub handler {
	...
	if ($paramRef->{'resetapcdatabase'}) {
		...
		Plugins::AlternativePlayCount::Plugin::resetAPCDatabase(1);
	} elsif ($paramRef->{'clearapcdatabase'}) {
		if ($callHandler) {
			$result = $class->SUPER::handler($client, $paramRef);
		}
